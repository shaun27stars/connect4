class GuyPlayer
  ...
  def take_turn(state, colour)
    ...
    0..6.each do |row|
      ...
      columns.each_with_index do |column,column_no|
        ...
        if counter == 3
          return (column_no + 1) if is_legal_move? (column_no + 1)
          return (examined_columns.first - 1) if is_legal_move? (examined_columns.first - 1)
        end
      end
    end
    ...
    while !is_legal_move?(random_number)
      random_number = rand(7)
    end
  ...
  def is_legal_move?(column)
    move = column
    if find_legal_moves.include? move
      return true
    else
      return false
    end
